...
function Rating({ rating }) {
  const stars = Array.from({ length: 5 }, (_, index) => {
    let colorClass = '';
    if (index < rating) {
      colorClass = 'star-filled'; 
    } else {
      colorClass = 'star-empty'; 
    }
    return <StarIcon key={index} className="star" filled={colorClass === 'star-filled' ? '#FF6060' : '#D8D8D8'} />;
  });

  return <div className='stars-list'>{stars}</div>;
}
